...
@section('page-title', 'Add Contact Page')
...
@endphp
<div class="admin-page-head"><div><a href="{{ route('admin.contact-pages.index') }}" class="admin-back-link">&larr; {{ trans('global.back_to_list') }}</a><h2 class="admin-page-title">Add Contact Page</h2></div></div>
<form method="POST" action="{{ route('admin.contact-pages.store') }}" class="form-card">@csrf
<div class="row">
@foreach($fields as $field)
<div class="col-md-6"><div class="field-group"><label>{{ ucwords(str_replace('_',' ',$field)) }}</label>@if(in_array($field,$textarea))<textarea name="{{ $field }}" class="form-control" rows="3">{{ old($field) }}</textarea>@else<input type="text" name="{{ $field }}" class="form-control" value="{{ old($field) }}">@endif</div></div>
@endforeach
...
</div><div class="form-actions"><a href="{{ route('admin.contact-pages.index') }}" class="btn-outline">Cancel</a><button class="btn-primary" type="submit"><i class="fas fa-save"></i> Save</button></div></form>
@endsection
