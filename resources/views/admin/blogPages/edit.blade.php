...
<div class="admin-page-head">
    <div>
        ...
        <p class="admin-page-subtitle">Update dynamic blog page content.</p>
    </div>
</div>

<form method="POST" action="{{ route('admin.blog-pages.update', $blogPage) }}" class="form-card">
    @csrf @method('PUT')
                ...
                <div class="col-md-6">
                    <div class="field-group">
                        ...
                        @if(in_array($field, $textarea))
                            <textarea name="{{ $field }}" class="form-control" rows="3">{{ old($field, $blogPage->$field) }}</textarea>
                        @else
                            <input type="text" name="{{ $field }}" class="form-control" value="{{ old($field, $blogPage->$field) }}">
                        @endif
                    </div>
                </div>
            @endforeach
    ...
    <div class="field-group">
        <label>Status</label>
        <select name="status" class="form-control"><option value="1" {{ old('status', $blogPage->status) ? 'selected' : '' }}>Active</option><option value="0" {{ !old('status', $blogPage->status) ? 'selected' : '' }}>Inactive</option></select>
    </div>
